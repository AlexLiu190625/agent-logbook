...
function ruleReferencesResolve(ctx) {
  const violations = [];
  const files = [{ rel: ws.LOGBOOK_FILE, abs: ctx.logbookPath, links: ctx.logbookLinks }].concat(
  ...
  for (const file of files) {
    for (const link of file.links) {
      ...
      // GitHub renders them. A path that only resolves from the logbook root is
...
function ruleDocumentsRegistered(ctx) {
  ...
  if (checkedDirs === 0) {
    ...
    notes.push('R3: LOGBOOK.md has no "Document index" heading, so no document counts as registered.');
  }
  return { violations, skipped: [], notes };
}
