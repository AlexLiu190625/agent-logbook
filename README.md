...
agent-ledger is a file format for writing that down, and a checker that stops
...
`ledger init` creates `LEDGER.md` and `journal/`. Entries go in daily files:
...
`ledger check` is the reason the format survives contact with real work. An
...
ledger check failed: 1 problem (R4:1) across 1 entry.
```

That line was not wrong, exactly. It was a guess wearing the clothes of a fact,
which is the failure that makes a ledger worse than no ledger at all. Moving it
...
ledger check failed: 1 problem (R5:1) across 1 entry.

Commit stopped: the ledger does not pass its own rules.
...
```
$ ledger check
ledger check passed: 1 entry in 1 journal file.
```

## Install

```
npm install --save-dev agent-ledger
```

Or run it without installing:

```
npx agent-ledger init
```
...
| --- | --- |
| `ledger init` | Creates `LEDGER.md` and `journal/` in the current directory, refusing to overwrite either if it already exists. |
| `ledger add [title]` | Appends a blank entry to today's journal file, dated from the system clock, creating the file if needed. |
| `ledger check` | Runs the five rules and exits non-zero on any violation, printing each one with its file, line, and what to do about it. |
...
| R3 | Every file under a registered directory (`reviews/` by default) has a row in the `LEDGER.md` document index. |
...
```
cp node_modules/agent-ledger/hooks/pre-commit .git/hooks/pre-commit
chmod +x .git/hooks/pre-commit
```

Run it in CI with `.github/workflows/ledger-check.yml` from this repository.
...
To teach an assistant the habit, copy `skills/ledger/SKILL.md` into the skills
...
`.ledgerrc.json` at the ledger root, entirely optional:
...
to check. A `.ledgerrc.json` that is not valid JSON is an error, so that a typo
...
does not verify. This repository keeps its own ledger under `journal/`, and its
