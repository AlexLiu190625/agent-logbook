...
const LOGBOOK_FILE = 'LOGBOOK.md';
const JOURNAL_DIR = 'journal';
const CONFIG_FILE = '.logbookrc.json';
const DEFAULT_REGISTERED_DIRS = ['reviews'];

// The logbook root is the nearest ancestor holding LOGBOOK.md, so the CLI and the
// pre-commit hook behave the same from any subdirectory.
function findRoot(startDir) {
  let dir = path.resolve(startDir);
  for (;;) {
    if (fs.existsSync(path.join(dir, LOGBOOK_FILE))) return dir;
